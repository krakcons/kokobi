...
export const learnersData = {
  ...
  update: async ({ id, moduleId, data }: UpdateLearner) => {
    ...
    const justCompleted =
      !learner.completedAt &&
      (learner.course.completionStatus === newLearner.status || isEitherStatus);
    ...
    if (justCompleted) {
      ...
      const i18n = await createI18n({ locale: courseModule?.language! });
          ...
          const courseInvites = learnerList
            ...
            .map((l) => {
              ...
              const learnerId = finalLearners.find(
                (l) => l.email === learner.email && l.courseId === course.id,
              )?.id!;
              return {
                ...course,
                learnerId,
        ...
        .map((learner) => {
          ...
          return {
            ...
            learnerId: finalLearners.find(
              (l) =>
                l.email === learner.email && l.courseId === learner.courseId,
            )?.id!,
            course,
            inviteLanguage: learner.inviteLanguage,
          };
  ...
  collectionInvite: async ({
    ...
    const courseInvites = courses.map((course) => {
      ...
      const courseTranslation = translate(course.translations, inviteLanguage);
