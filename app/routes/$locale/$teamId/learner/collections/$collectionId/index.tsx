...
export const Route = createFileRoute(
	...
	loader: async ({ params }) => {
		...
		if (!connection) {
			throw redirect({
				to: "/$locale/$teamId/learner/collections/$collectionId/request",
				from: Route.fullPath,
				params: {
		...
		if (
			...
			throw redirect({
				to: "/$locale/$teamId/learner/collections/$collectionId/invite",
				from: Route.fullPath,
				params: {
					collectionId: params.collectionId,
				},
			...
			<div className="flex flex-col gap-8 w-full">
				...
				<div className="flex flex-col gap-4">
					...
					{courses?.map(({ id, name, description }) => (
						<Link
							key={id}
							to="/$locale/$teamId/learner/courses/$courseId"
							from={Route.fullPath}
							params={{
								courseId: id,
							}}
