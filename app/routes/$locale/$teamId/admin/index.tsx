...
export const Route = createFileRoute("/$locale/$teamId/admin/")({
			...
			{courses.length > 0 ? (
				courses.map((c) => (
					<Link
						key={c.id}
						from={Route.fullPath}
						to="/$locale/$teamId/admin/courses/$id/learners"
						params={(p) => ({ ...p, id: c.id })}
					>
			...
			{collections.length > 0 ? (
				collections.map((c) => (
					<Link
						key={c.id}
						from={Route.fullPath}
						to="/$locale/$teamId/admin/collections/$id/learners"
						params={(p) => ({ ...p, id: c.id })}
					>
						<Card className="flex justify-between items-center">
							<CardHeader>
				...
				<Link from={Route.fullPath} to="/$locale/$teamId/admin">
					<Card className="flex justify-between items-center">
						<CardHeader>
